# vllm-project/vllm#21771: [Bug]:  prefix cache usage is always 0% when passing text prompt + image embedding as Input

| 字段 | 值 |
| --- | --- |
| Issue | [#21771](https://github.com/vllm-project/vllm/issues/21771) |
| 状态 | closed |
| 标签 | bug;stale |
...
> [Bug]:  prefix cache usage is always 0% when passing text prompt + image embedding as Input
...
- [Scheduler 与请求状态生命周期](../patterns/scheduler_request_lifecycle.md) - 分数 4: age is always 0% when passing text prompt + image embedding as Input bug;stale ### Your current environment ### 🐛 Describe the bug ``` def call_vllm_api_with_embeds(image_embedding, question="What's in this image?", mod...
...
- [Bitwise 确定性与数值等价](../patterns/bitwise_determinism_equivalence.md) - 分数 2: When I pass the text prompt and image embedding as the input, the Prefix cache hit rate is always 0.0%. This is shown in the vllm console output. My command to run the vllm server is: ``` CUDA_VISIBLE_DEVICES="6,7" VLLM...
