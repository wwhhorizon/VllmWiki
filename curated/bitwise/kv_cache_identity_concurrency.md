...
- [#37076](https://github.com/vllm-project/vllm/issues/37076) 现已抓到 linked PR [#37152](https://github.com/vllm-project/vllm/pull/37152)，issue 评论还补了更清晰的复现脚本和 patch 解释：`cache_full_blocks()` 把 newly registered block 提前暴露给 `get_cached_block()`，会让同一步后续请求命中尚未完成 GPU forward 的块；`#37152` 用 `_blocks_registered_this_step` 在 `new_step_starts()` 前将其视为 miss。这个方向足以支持“same-step registration visibility 是 KV identity 契约”这条 insight，但 PR 仍 open/unmerged，只有最小代码修改和 bot review，没有 merged regression test 或 maintainer resolution，因此仍保持 defer。
...
- 继续追踪 `#37152` 是否合并；重点看是否补出正式 regression test，以及 same-step miss gate 是否保留在最终 patch 中。当前 issue comment 已给出更清晰复现和 patch 解释，但还没有 maintainer resolution。
