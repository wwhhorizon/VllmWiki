...
| --- | --- | --- | --- | --- |
| [#38991](https://github.com/vllm-project/vllm/issues/38991) | quant/dtype loading identity | defer | #38991 自身仍是 open/0 comments，截至 2026-06-21 仍查不到直接引用该 issue 的 PR。已合入的 [#43163](https://github.com/vllm-project/vllm/issues/43163) / [#43464](https://github.com/vllm-project/vllm/pull/43464) 将 unified-memory loader 推进为上游接受的通用 correctness fix（runai_safetensors_weights_iterator 在 yield 前 clone），但仅覆盖 Llama4 family-level 样例而非全局 closure；[#44430](https://github.com/vllm-project/vllm/issues/44430) / [#44645](https://github.com/vllm-project/vllm/pull/44645) 说明 clone 带来的 host-RAM 回归应由 streaming loader 吸收。当前缺口收缩为：#38991 本体的 direct issue-level closure 与正式 regression test。 | 优先把缺口收敛为"缺 dedicated regression/maintainer closure"，继续区分 Llama4 已 landed 样例与其他模型路径；除非出现反证，否则不再默认转向 sync/ownership 修法探索。 |
