# vllm-project/vllm#5636: Cohere model output speed about 5 tokens per second on vllm.

| 字段 | 值 |
| --- | --- |
| Issue | [#5636](https://github.com/vllm-project/vllm/issues/5636) |
| 状态 | closed |
| 标签 | bug;stale |
...
- [构建、依赖与打包](../patterns/build_dependency_packaging.md) - 分数 7: Your current environment Collecting environment information... PyTorch version: 2.2.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM used to build PyTorch: N/A OS: Ubuntu 22.04.4 LTS (x86_64) GCC vers...
...
- [模型格式与 Adapter 路径](../patterns/model_format_adapter.md) - 分数 4: Cohere model output speed about 5 tokens per second on vllm. bug;stale ### Your current environment Collecting environment information... PyTorch version: 2.2.1+cu121 Is debug build: False CUDA used to build PyTorch: 12...
- [Scheduler 与请求状态生命周期](../patterns/scheduler_request_lifecycle.md) - 分数 2: Cohere model output speed about 5 tokens per second on vllm. bug;stale ### Your current environment Collecting environment information... PyTorch version: 2.2.1+cu121 Is debug build: False CUDA used to build PyTorch: 12...
- [Backend 路由与 Fallback](../patterns/backend_routing_fallback.md) - 分数 1: nccl-cu12==2.19.3 [pip3] torch==2.2.1 [pip3] transformers==4.40.1 [pip3] triton==2.2.0 [pip3] vllm-nccl-cu12==2.18.1.0.4.0 [conda] numpy 1.26.4 pypi_0 pypi [conda] nvidia-nccl-cu12 2.19.3 pypi_0 pypi [conda] torch
