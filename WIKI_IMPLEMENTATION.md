...
状态：持续更新的实现文档。  
参考状态：KDA 与 KernelWiki 已从源仓库阅读；具体调研见 [KDA 调研笔记](KDA_RESEARCH_NOTES.md)。
...
KDA 的核心是 agent-driven optimization loop：定义 task contract、检查源材料、写 draft/plan、迭代 candidate、验证、记录证据，最后才 promotion。VllmWiki 把这个循环用于知识构建。
...
| `data/version-claims.yaml` | [data/version-claims.yaml](data/version-claims.yaml)：数据快照和 KDA 调研版本声明 |
