...
def build_home(
    ...
    lines = [
        ...
        f"- 按编号去重后的原始 issue 数：{len(issues):,}",
        f"- Operator/kernel 候选 case 行数：{raw_counts['cases_csv'] - 1:,}",
        f"- Operator/kernel 候选 issue 去重数：{len(cases):,}",
        f"- 原始 PR JSONL 行数：{raw_counts['prs_raw']:,}",
        f"- PR 表按编号去重行数：{len(prs):,}",
        f"- 本地存在 issue-PR 链接的 issue 数：{linked_issues:,}",
        f"- 本地缺失评论正文但有评论计数的 issue 数：{comment_missing:,}",
    ...
    for area, numbers in sorted(domain_cases.items(), key=lambda item: (-len(item[1]), item[0])):
        title = WORK_AREA_NAMES.get(area, area)
        lines.append(f"- [{title}](domains/{area}.md): {len(numbers):,} 个候选 case")
    lines.extend(["", "## 优化模式", ""])
    for key, spec in PATTERNS.items():
        lines.append(
            f"- [{spec['title']}](patterns/{key}.md): "
            f"{len(pattern_issue_hits[key]):,} 个 issue 命中，{len(pattern_pr_hits[key]):,} 个 PR 命中"
        )
    lines.extend([
        "",
        "## 优先焦点：Bitwise 与数值等价",
