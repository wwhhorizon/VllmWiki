# Prefix Cache 等价 Evidence Appendix

状态：curated public evidence summary。

父页：[../prefix_cache_equivalence.md](../prefix_cache_equivalence.md)。
...
- `#33123` 的用户可见症状非常直接：同一 deterministic prompt，在首次 cache miss 与后续 warmed cache hit 之间会分叉到不同 token，因此它首先不是“cache 性能差异”，而是 prefix-cache path 改变了可见语义。
- `#40179` 的 patch 方向也很聚焦：不是粗暴禁用 prefix caching，而是在 scheduler 层把 cache-miss prefill 拆到最后一个 cached block boundary，尽量让 suffix-prefill 的 GEMM `M` 与 cache-hit path 对齐，从而把 divergence 收口到几何差异本身。
- `#40179` 的 review 风险同样说明验证不能只测 happy path：`request.num_output_tokens > 0` 会漏掉 resumed request，而 `remainder == 0` 会漏掉 block-aligned prompt 与 final-token scheduler 约束，所以“普通重复请求通过”不等于 split boundary 已完整闭环。
...
| 场景 | 主要目的 | 最低合格证据 |
| --- | --- | --- |
| no-prefix baseline | 给 full-prefill 几何一个参考面 | `temperature=0` token/logprob 作为基线 |
| 首次 cache miss | 比较 full-prefill 与 deterministic split 前后 | token equality；必要时记录 suffix GEMM `M` |
| warmed cache hit | 验证 hit path 不改变可见 token | token equality；必要时检查 logprob ranking |
| resumed request | 防止 `num_output_tokens > 0` 早退漏掉 split | token equality + split boundary 命中 |
| block-aligned prompt | 防止 `remainder == 0` 漏掉 final-token-aware boundary | token/logprob equality + final-token 约束说明 |
| `fp32` 对照 | 判断是否是低精度数值路径放大 | divergence 在 `fp32` 下是否消失 |
| `VLLM_BATCH_INVARIANT=1` 对照 | 判断是否属于 batch/kernel geometry 分流 | divergence 在 BI mode 下是否消失 |
...
- [#43650](https://github.com/vllm-project/vllm/pull/43650) 是 open PR（7 comments、0 review comments、1 changed file），揭示 MTP/eagle 场景下 Full-attention 与 Mamba/GDN 的 cache hit search 边界差异：Full-attention cache hit lookup 会多匹配一个 block 然后丢弃最后一个（部分 accept），但 Mamba/GDN state blocks 的结构是 `[null, ..., state]`，pop 最后一个 block 会移除 state 本身。修复方向是对 eagle/MTP 场景把搜索边界减 1（不包含 final block），而不是匹配后 pop。这和 `#34865/#34874` 的 Mamba metadata pointer 是同一机制家族，但关注点是 cache hit search 边界而非 metadata buffer。PR 仍 open/unmerged，不能写成 landed fix。
...
- 追踪 `#43650` 是否合并；它揭示 MTP/eagle 场景下 Full-attention 与 Mamba/GDN 的 cache hit search 边界差异——Full-attention 多匹配一个 block 后 pop，但 Mamba/GDN state blocks 的 pop 会移除 state。若合并，可作为 prefix-cache hit search 在 hybrid 模型上的 landed boundary。
