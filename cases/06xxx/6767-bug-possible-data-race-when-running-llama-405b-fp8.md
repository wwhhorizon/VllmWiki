# vllm-project/vllm#6767: [Bug]: Possible data race when running Llama 405b fp8

| 字段 | 值 |
| --- | --- |
| Issue | [#6767](https://github.com/vllm-project/vllm/issues/6767) |
| 状态 | closed |
| 标签 | bug |
...
#6788 [Bugfix] Add synchronize to prevent possible data race

## 候选优化模式

- [构建、依赖与打包](../patterns/build_dependency_packaging.md) - 分数 7: r current environment ``` Collecting environment information... PyTorch version: 2.3.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM used to build PyTorch: N/A OS: Ubuntu 22.04.4 LTS (x86_64) GCC ver...
...
- [模型格式与 Adapter 路径](../patterns/model_format_adapter.md) - 分数 5: [Bug]: Possible data race when running Llama 405b fp8 bug ### Your current environment ``` Collecting environment information... PyTorch version: 2.3.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM u...
- [Dtype、量化与 Scale 路径](../patterns/dtype_quantization_path.md) - 分数 4: [Bug]: Possible data race when running Llama 405b fp8 bug ### Your current environment ``` Collecting environment information... PyTorch version: 2.3.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM u...
...
| --- | --- | ---: | --- | --- |
| [#6788](https://github.com/vllm-project/vllm/pull/6788) | mentioned | 0.6 | [Bugfix] Add synchronize to prevent possible data race | sue that CSAN complains about when we set `TORCH_CUDA_SANITIZER=1` in #6767, but does not resolve the underlying problem, unfortunately. --- <details> <!-- inside this <details> |
