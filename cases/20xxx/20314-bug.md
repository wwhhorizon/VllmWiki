# vllm-project/vllm#20314: [Bug]:

| 字段 | 值 |
| --- | --- |
| Issue | [#20314](https://github.com/vllm-project/vllm/issues/20314) |
| 状态 | closed |
| 标签 | bug |
...
- [模型格式与 Adapter 路径](../patterns/model_format_adapter.md) - 分数 3: installed vllm==0.9.0. -with the newest function **load_bytes** of /vllm/multimodal/video.py ``` @classmethod def load_bytes(cls, data: bytes, num_frames: int = -1) -> npt.NDArray: import cv2 backend = cls().get_cv2_vid...
