...
| [#40179](https://github.com/vllm-project/vllm/pull/40179) | PR body 明确说明 cache-miss full prefill 的 GEMM `M=N`，cache-hit suffix prefill 的 `M=N % block_size`；不同 `M` 会改变 ROCm/Tensile tiling 和 BF16 accumulation order。changed files 包含 cache config、scheduler split 和 e2e test。review comment 进一步指出 resumed request 和 block-aligned prompt 可能绕过 split 逻辑。 | deterministic mode 下要让 cache miss 与 cache hit 的 suffix GEMM geometry 对齐；但 split 逻辑还必须覆盖 preemption/resume 和 exact block boundary。 |
...
- 调度边界层：覆盖 resumed request、block-aligned prompt、至少计算 final token 的 vLLM scheduler 约束。
...
- review comment 暴露的 resumed/block-aligned 风险是实现边界，不等价于 PR 已完全失效；promotion 时应写成需要测试覆盖的 boundary。
