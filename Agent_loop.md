...
状态：single source of truth。作用：规定每轮 agent 如何从证据走向 wiki 结论。与 `docs/agent_loop.md` 保持一致。
## 输入
每轮先读取：
1. [维护规则](docs/maintenance.md)：仓库边界、证据规则、promotion 条件、质量门。
...
## Sub-agent 协作模式
每轮分析使用以下 sub-agent 角色：
1. **搜索员 (explorer)**：并行读取 issue/PR JSON 原文、linked PR、review comments、changed files。输出原始事实摘要，不做结论判断。
2. **审查员 (reviewer)**：复核搜索员的输出，对照原文检查：
   - 引用的 root cause 是否在原文中出现（防幻觉）
   - 引用的 fix pattern 是否在 diff/commit 中被验证
   - 引用的 test coverage 是否实际存在
   - 标记任何无原文支撑的推测
3. **主 agent**：整合搜索员和审查员输出，做出决策并更新文档。
...
## 低可信度/不相关条目清理
如果某条目：
- 原文与 bitwise/deterministic 主线无关（仅关键词偶然命中）
- 原文可信度极低，无法提取任何有效机制
- 属于错误的自动分类
则**从总条目 CSV 中删除该条目**，并在 ledger 中标记 `exclude`。
...
- `next.md` 默认拆成主线核心缺口和辅助边界队列。主线只放直接阻塞 bitwise 主机制闭环的项。
...
不要把 raw/generated/audit 材料提交到 GitHub。静态规则只放 [maintenance.md](docs/maintenance.md)，术语只放 [glossary.md](docs/glossary.md)。
