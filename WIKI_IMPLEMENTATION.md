...
机制分类是开放 taxonomy，不是封闭全集。已有机制页只能代表当前证据下的稳定压缩，不能为了维持目录稳定把新问题硬塞进旧机制。

当一个 claim 无法自然映射到现有机制时，先保持 `candidate`、`defer` 或 `blocked`，并在 ledger / next queue 中写清分类缺口。只有同时满足以下条件，才新增机制页或机制族：

- 现有机制无法解释它的 root cause、fix pattern 和 verification contract。
- 它不是单个 backend、模型或硬件的孤立特例，而是可能重复出现的工程机制。
- 它有可引用的 source evidence：issue/PR/comment/diff/test 至少能支撑现象和根因方向。
- 新分类能减少混淆，避免把不同修法、不同验证契约的 claim 混在同一机制页。

如果一个 claim 只是已有机制的边界、反例、support gate、workaround 或 test placement，不应新增机制页；应写入对应机制页的 `适用边界` / `仍需补证`，或进入专题 `next.md`。
