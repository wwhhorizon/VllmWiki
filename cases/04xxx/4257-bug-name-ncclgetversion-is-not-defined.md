# vllm-project/vllm#4257: [Bug]: name 'ncclGetVersion' is not defined

| 字段 | 值 |
| --- | --- |
| Issue | [#4257](https://github.com/vllm-project/vllm/issues/4257) |
| 状态 | closed |
| 标签 | bug |
...
- [构建、依赖与打包](../patterns/build_dependency_packaging.md) - 分数 9: [Bug]: name 'ncclGetVersion' is not defined bug ### Your current environment ```text Collecting environment information... PyTorch version: 2.2.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM used to...
...
- [模型格式与 Adapter 路径](../patterns/model_format_adapter.md) - 分数 4: fined bug ### Your current environment ```text Collecting environment information... PyTorch version: 2.2.1+cu121 Is debug build: False CUDA used to build PyTorch: 12.1 ROCM used to build PyTorch: N/A OS: Ubuntu 22.04.3...
- [Backend 路由与 Fallback](../patterns/backend_routing_fallback.md) - 分数 1: numpy==1.26.4 [pip3] nvidia-nccl-cu12==2.19.3 [pip3] torch==2.2.1 [pip3] triton==2.2.0 [conda] numpy 1.26.4 pypi_0 pypi [conda] nvidia-nccl-cu12 2.19.3 pypi_0 pypi [conda] torch 2.2.1
- [Dtype、量化与 Scale 路径](../patterns/dtype_quantization_path.md) - 分数 1: xgetbv1 xsaves cqm_llc cqm_occup_llc cqm_mbm_total cqm_mbm_local avx512_bf16 wbnoinvd dtherm ida arat pln pts hwp hwp_act_window hwp_epp hwp_pkg_req avx512vbmi umip pku ospke waitpkg avx512_vbmi2 gfni vaes vpclmulqdq av...
