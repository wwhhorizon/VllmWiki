...
| [#42125](https://github.com/vllm-project/vllm/issues/42125) | runtime LoRA / prefix cache identity | defer | 现已抓到 closed 的 [#42495](https://github.com/vllm-project/vllm/pull/42495) 和更新的 open PR [#45981](https://github.com/vllm-project/vllm/pull/45981)。与早先的 per-process counter 思路不同，`#45981` 已把修法收敛到 loader-effective 的内容派生 `lora_cache_key`：选中的 weight file、canonicalized `adapter_config.json`、`is_3d_lora_weight` 与 tensorizer config 一起进 hash，并由 serving load path 与 `process_inputs` backstop 共用 `ensure_lora_cache_key` 填充；block-hash extra keys 也拆成 `("lora", name) + ("lora_identity", id)` 两层。测试除 same-name different-path、same-path different-cache-key、same-content same-key、config/weight 变化改 key 外，还覆盖了 safetensors 优先级、length framing collision、relative path path-only fallback。这条线在技术机制上已经很成熟；剩余缺口主要是 HF Hub、relative/remote path、tensorizer/shared-nothing、多节点 rolling-upgrade、external KV connector、TOCTOU，以及截至 2026-06-20 仍缺 substantive maintainer review。 | 优先追踪 `#45981` 的 merge 与后续 review；确认 unload/reload、same-path content change、block-hash extra key、path-only warning 与 old EngineCore peer/rolling-upgrade 边界是否保留，并把未覆盖 source/TOCTOU/external-KV 边界继续留在 defer。 |
| [#44250](https://github.com/vllm-project/vllm/issues/44250) | external KV / LoRA identity | defer | 现已抓到 linked PR [#45549](https://github.com/vllm-project/vllm/pull/45549)：它把 `lora_name` 折进 `LMCacheMPConnector` 的 internal cache salt，并补 unit test；issue comment 还指向上游 LMCache draft PR [#2962](https://github.com/LMCache/LMCache/pull/2962) 的 MP 路径修复。新增 insight 是：external KV 的闭环不是单个 salt 字段，而是 vLLM connector 的 internal salt、LMCache 的 chunk-hash / `CacheEngineKey.tags` / L2 object key 三层都要一致带上 adapter identity。相比 `#42125`，这条线当前更像“跨仓 schema 还未统一的系统级缺口”：`#45549` 只补了 vendored MP connector salt 层，LMCache `#2962` 仍 open/dirty 且主要覆盖 V1 path。 | 继续追踪 `#45549` 与 LMCache `#2962` 的合并；确认 MP lookup/store key、vendored connector、chunk-hash/CacheEngineKey.tags/L2 object key、same-adapter hit 保留与跨 adapter negative regression test 同步闭环。 |
