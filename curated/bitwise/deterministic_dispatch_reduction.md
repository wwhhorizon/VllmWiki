...
| [#35569](https://github.com/vllm-project/vllm/issues/35569), [#39849](https://github.com/vllm-project/vllm/pull/39849) | `#35569` 报告 Qwen3-VL reranker pooling 在 ROCM_ATTN 上有系统性 score deviation：gfx950 约 8.45%，gfx942 约 3.66%，而 ROCM_AITER_FA/TRITON_ATTN/FLEX_ATTENTION 明显更低。open PR `#39849` 在 gfx9 上把 `gqa_ratio in (2, 4)` 的 ROCM_ATTN native `mfma4` paged-attention path 路由回 Triton，并用 fake AMD SMI 单测覆盖 gfx950 head_size=128/gqa_ratio=2、gfx942 head_size=64/gqa_ratio=4 和 gqa_ratio=5 保持 native path。 | backend-specific drift 应作为 correctness boundary；当前 fix 是 selector workaround，把 known-bad shape 退回更稳定 fallback。PR 仍 open/unmerged，缺 patched reranker e2e score 回归，不能写成 landed root-cause fix。 |
...
| [#30018](https://github.com/vllm-project/vllm/pull/30018) | FA2 path 通过 `num_splits` 控制 split geometry；PR body、测试矩阵和 review 同时把 LoRA `split_k` 作为 deterministic reduction 目标，但本地 evidence 仍需补最终 landed-code 子路径。早期 review 指出硬编码 `num_splits=0` 会破坏 CUDA graph metadata，后续作者明确该 PR 测试时启用 `enforce_eager`，不支持 CUDA graph。 | split 参数是 dispatch/reduction contract，不是实现细节；但 LoRA landed-code 与 CUDA graph compatibility 都必须单独验证。 |
...
- [#42650](https://github.com/vllm-project/vllm/pull/42650) 已 merged，但它只覆盖 metadata head-count source；`#41651` 评论明确还提到独立的 Triton FP8 `k_scale/v_scale` 问题（#42580），因此不能写成 TRITON_ATTN FP8 KV 全部由该 PR 修复。后续评论还暴露 Gemma4 MTP target/draft Q-head 混入同一 attention group 的断言风险，说明 grouping key 仍需后续处理。
...
- 追踪 `#42650` 后续是否修复 Gemma4 MTP grouping key，让 target/draft 不同 Q-head 的 layers 不再进入同一个 attention group；同时确认 #42580 的 FP8 scale 修复状态。
