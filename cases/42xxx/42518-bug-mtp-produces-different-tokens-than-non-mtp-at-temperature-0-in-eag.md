# vllm-project/vllm#42518: [Bug]: MTP produces different tokens than non-MTP at temperature=0 in eager mode

| 字段 | 值 |
| --- | --- |
| Issue | [#42518](https://github.com/vllm-project/vllm/issues/42518) |
| 状态 | closed |
| 标签 |  |
...
- [Backend 路由与 Fallback](../patterns/backend_routing_fallback.md) - 分数 3: mpute for verified positions KV, or extend batch invariance to attention backend dispatch correctness activation_norm;attention_kv_cache;gemm_linear;model_support;sampling_logits;speculative_decoding activation;attentio...
- [构建、依赖与打包](../patterns/build_dependency_packaging.md) - 分数 3: `enforce_eager=True`. The first mismatch occurs at a consistent, reproducible position (e.g., pos 99 for "Hello, my name is"). With `enforce_eager=False` (CUDA graph), outputs are 100% identical. I'd be happy to submit...
