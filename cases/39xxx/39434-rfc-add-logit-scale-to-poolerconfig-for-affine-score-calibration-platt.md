# vllm-project/vllm#39434: [RFC]: Add logit_scale to PoolerConfig for Affine Score Calibration (Platt Scaling)

| 字段 | 值 |
| --- | --- |
| Issue | [#39434](https://github.com/vllm-project/vllm/issues/39434) |
| 状态 | closed |
| 标签 | RFC |
...
- [模型格式与 Adapter 路径](../patterns/model_format_adapter.md) - 分数 2: [RFC]: Add logit_scale to PoolerConfig for Affine Score Calibration (Platt Scaling) RFC ### Motivation. vLLM's `PoolerConfig` already supports `logit_bias` for classification models, enabling a bias offset on raw logits...
- [Dtype、量化与 Scale 路径](../patterns/dtype_quantization_path.md) - 分数 1: [RFC]: Add logit_scale to PoolerConfig for Affine Score Calibration (Platt Scaling) RFC ### Motivation. vLLM's `PoolerConfig` already supports `logit_bias` for classification models, enabling a bias offset on raw logits...
