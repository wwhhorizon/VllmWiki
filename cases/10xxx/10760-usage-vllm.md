# vllm-project/vllm#10760: [Usage]: 可以在vllm的日志打印中加入模型的输出吗？因为请求端有的时候看不到结果，但是模型已经推理结束了，想在服务端看一下模型的输出

| 字段 | 值 |
| --- | --- |
| Issue | [#10760](https://github.com/vllm-project/vllm/issues/10760) |
| 状态 | closed |
| 标签 | usage;stale |
...
- [Scheduler 与请求状态生命周期](../patterns/scheduler_request_lifecycle.md) - 分数 1: e]: 可以在vllm的日志打印中加入模型的输出吗？因为请求端有的时候看不到结果，但是模型已经推理结束了，想在服务端看一下模型的输出 usage;stale ### Your current environment ```text The output of `python collect_env.py` ``` ### How would you like to use vllm I want to run inference of...
