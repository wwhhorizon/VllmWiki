...
- 继续追踪 `#45549` 是否合并，并确认它与上游 LMCache MP 路径修复同步落地；仅 vLLM vendored connector 的 internal salt 合并还不够，LMCache MP 的 chunk-hash / `CacheEngineKey.tags` / L2 object key schema 也必须闭环。相较之下，后续 adapter identity 深读仍应优先给 `#45981`，除非 LMCache `#2962` 或 `#45549` 出现新的 maintainer 决策。
