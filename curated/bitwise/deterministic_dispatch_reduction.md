...
| [#42240](https://github.com/vllm-project/vllm/pull/42240) | AITER FP8 block-scaled GEMM 的 CSV dispatcher 可能选择 `splitK >= 2`；CK split-K reduction 通过 atomic floating-point adds 累加 partial K products。PR 在 CK path 强制 `splitK=0`。 | split-K / atomic reduction 是 deterministic contract 的直接控制点。 |
...
- 对 split-K/atomic 修复，要记录 hardware、dtype、backend、kernel config 和 dispatcher source。

## 适用边界

- [#42240](https://github.com/vllm-project/vllm/pull/42240) 是 scoped fix：强制 `splitK=0` 绕过 CK reduction 问题，直到上游修复。
